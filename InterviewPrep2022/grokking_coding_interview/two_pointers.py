...
def triplet_helper(
    arr: List[int], target_sum: int, first: int, triplets: List[List[int]]
):
    right = len(arr) - 1
    left = first + 1
    while left < right:
...
assert triplet_with_smaller_sum_list([-1, 4, 2, 1, 3], 5) == [
    [-1, 1, 4],
    [-1, 1, 3],
    [-1, 1, 2],
    [-1, 2, 3],
...
def search_helper(
    arr: List[int], target_sum: int, left: int, triplets: List[List[int]]
) -> None:
    right = len(arr) - 1
    while left < right:
        current_sum = arr[left] + arr[right]
